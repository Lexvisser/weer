...
export async function stuurMailAlarm({ id, titel, bericht, url, lat, lon, gebiedPolygon, to }) {
  ...
  const html = attachments.length
    ? `<div style="font-family:sans-serif;white-space:pre-wrap;">${tekst.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</div><img src="cid:gebiedkaart" alt="Kaart met gebied" style="max-width:100%;border-radius:8px;margin-top:12px;" />`
    : undefined;
