...
const ZEE_GEBIED_MAX_ZOOM = 6; // 2026-09-17, derde ronde: Lex wil ze alleen bij een RUIM overzicht ("pas als ik heel UK kan zien"), dus juist andersom dan 10 -> 12; heel Groot-Brittannië in beeld is ongeveer zoom 6
...
async function tekenNwr() {
  ...
  for (const s of stations) {
    ...
    marker.on('click', (e) => {
      ...
      if (e.originalEvent?.target?.closest?.('.nwr-pin-tekst')) { nwrPaneelToon(s.id); return; }
...
let nwrTeksten = new Map(); // stationId -> laatste /api/radio-tekst-blok van die zender
...
// 2026-09-10 (eind): de kaart wordt pas gewisseld als de nieuwe data er is —
// eerst maakte de klik hem meteen leeg, en bij een mislukte aanvraag zag je
// dus niets, zonder melding. Een antwoord dat te laat komt (je klikte inmiddels
...
// Metingen komen elk uur binnen; tien minuten verversen is ruim genoeg.
...
async function nwrTekstVervers() {
  ...
  if (l?.actief && NWR_SPELER_TEKST_EL && !NWR_SPELER_TEKST_EL.textContent.includes('verstaat')) NWR_SPELER_TEKST_EL.textContent += ` · verstaat tot ${new Date(l.tot).toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit', hour12: false })}`;
}
...
function nwrPaneelToon(stationId) {
  if (stationId) nwrPaneelStation = stationId;
  nwrPaneelOpen = true;
...
const STATIONS_STRAAL_KM = 300; // 2026-09-07: heel NL incl. Wadden en Noordzeeplatforms (Lex: "ik wil Texel en de rest ook wel zien")
...
function knmiVakHtml(s) {
  ...
  const tendensHtml = tendens ? `<span class="station-tendens" title="Druk ${tendens.tekst} hPa in 3 uur">${tendens.tekst}<small>/3u</small></span>` : ''; // zonder pijltje, in pil én popup (Lex, 2026-09-07): het teken zegt genoeg
...
function renderMap(signalen) {
  ...
  const teTonenSignalen = vliegModusActief || kaartVolgType
    ? []
    // 2026-09-06, Lex: Met Office gale warnings (detail.zeekaart, zie
    // metOfficeGaleWarnings.js) op BEIDE kaarten -- op de Zeekaart erbij
    // naast NAVTEX, en gewoon ook op de gewone kaart.
    : zeeModusActief
